...
/**
 * SCV2 REST API controller class.
 */
class SCV2_API_Controller {

	/**
	 * Endpoint namespace.
	 */
	protected $namespace = 'scv2/v1';
	...
	public function register_routes() {
		...
		// Get Cart in Session - scv2/v1/get-cart/1654654321 (GET)
		register_rest_route(
			$this->namespace,
		...
		// Get Customers Cart saved via Persistent Cart - scv2/v1/get-cart/customer/1 (GET)
		register_rest_route(
			$this->namespace,
	...
	public function get_permission_check() {
		if ( ! current_user_can( 'administrator' ) ) {
			return new WP_Error( 'scv2_cannot_read_cart', __( 'Cannot read cart!', 'cart-rest-api-for-woocommerce' ), array( 'status' => 401 ) );
		}
	...
	public function get_cart( $data = array(), $cart_item_key = '' ) {
		...
		do_action( 'scv2_get_cart', $cart_contents );
	...
	public function get_cart_customer( $data = array() ) {
		if ( empty( $data['id'] ) ) {
			return new WP_Error( 'scv2_customer_missing', __( 'Customer ID is required!', 'cart-rest-api-for-woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function return_cart_contents( $data = array(), $cart_contents = array(), $cart_item_key = '', $from_session = false ) {
		if ( SCV2_Count_Items_Controller::get_cart_contents_count( array( 'return' => 'numeric' ), $cart_contents ) <= 0 || empty( $cart_contents ) ) {
			/**
			 * Filter response for empty cart.
			 */
			$empty_cart = apply_filters( 'scv2_return_empty_cart', array() );
		...
		foreach ( $cart_contents as $item_key => $cart_item ) {
			...
			if ( ! $_product->is_purchasable() ) {
				...
				$cart_contents[ $item_key ]['product_name']  = apply_filters( 'scv2_product_name', $_product->get_name(), $_product, $cart_item, $item_key );
				$cart_contents[ $item_key ]['product_title'] = apply_filters( 'scv2_product_title', $_product->get_title(), $_product, $cart_item, $item_key );
				...
				if ( $show_thumb ) {
					...
					$thumbnail_id  = apply_filters( 'scv2_item_thumbnail', $product_thumbnail_id, $cart_item, $item_key );
					$thumbnail_src = wp_get_attachment_image_src( $thumbnail_id, apply_filters( 'scv2_item_thumbnail_size', 'woocommerce_thumbnail' ) );

					/**
					 * Filters the source of the product thumbnail.
					 */
					$thumbnail_src = apply_filters( 'scv2_item_thumbnail_src', $thumbnail_src[0], $cart_item, $item_key );
				...
				$cart_contents = apply_filters( 'scv2_cart_contents', $cart_contents, $item_key, $cart_item, $_product );
			}
		}
		 ...
		 */
		if ( $from_session ) {
			return apply_filters( 'scv2_return_cart_session_contents', $cart_contents );
		} else {
			return apply_filters( 'scv2_return_cart_contents', $cart_contents );
		}
	...
	public function get_cart_in_session( $data = array() ) {
		...
		if ( empty( $cart_key ) ) {
			return new WP_Error( 'scv2_cart_key_missing', __( 'Cart Key is required!', 'cart-rest-api-for-woocommerce' ), array( 'status' => 404 ) );
		}

		// Get the cart in the database.
		$handler = new SCV2_Session_Handler();
		$cart    = $handler->get_cart( $cart_key );

		// If no cart is saved with the ID specified return error.
		if ( empty( $cart ) ) {
			return new WP_Error( 'scv2_cart_in_session_not_valid', __( 'Cart in session is not valid!', 'cart-rest-api-for-woocommerce' ), array( 'status' => 404 ) );
		}
	...
	protected function validate_product_id( $product_id ) {
		...
		if ( empty( $product_id ) ) {
			...
			return new WP_Error( 'scv2_product_id_required', $message, array( 'status' => 404 ) );
		}

		if ( ! is_numeric( $product_id ) ) {
			$message = __( 'Product ID must be numeric!', 'cart-rest-api-for-woocommerce' );
			SCV2_Logger::log( $message, 'error' );
			return new WP_Error( 'scv2_product_id_not_numeric', $message, array( 'status' => 405 ) );
		}
	...
	protected function validate_quantity( $quantity ) {
		if ( ! is_numeric( $quantity ) ) {
			return new WP_Error( 'scv2_quantity_not_numeric', __( 'Quantity must be numeric!', 'cart-rest-api-for-woocommerce' ), array( 'status' => 405 ) );
		}
	...
	protected function validate_variable_product( $variation_id, $variation, $product ) {
		...
		foreach ( $variable_product_attributes as $attribute ) {
			...
			if ( isset( $variation[ wc_variation_attribute_name( $attribute['name'] ) ] ) ) {
				...
				return new WP_Error( 'scv2_invalid_variation_data', $message, array( 'status' => 400 ) );
			}
		...
		if ( ! empty( $missing_attributes ) ) {
			...
			return new WP_Error( 'scv2_missing_variation_data', $message, array( 'status' => 400 ) );
		}
	...
	protected function get_variation_id_from_variation_data( $variation, $product ) {
		...
		if ( empty( $variation_id ) ) {
			...
			return new WP_Error( 'scv2_no_variation_found', $message, array( 'status' => 400 ) );
		}
	...
	protected function get_variable_product_attributes( $product ) {
		...
		if ( ! $product || 'trash' === $product->get_status() ) {
			...
			return new WP_Error( 'scv2_cart_invalid_parent_product', $message, array( 'status' => 403 ) );
		}
	...
	protected function validate_product( $product_id = null, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array(), $product_type = '' ) {
		...
		if ( ! $product || 'trash' === $product->get_status() ) {
			 ...
			 */
			$message = apply_filters( 'scv2_product_does_not_exist_message', $message, $product );

			return new WP_Error( 'scv2_product_does_not_exist', $message, array( 'status' => 404 ) );
		}
		...
		$passed_validation = apply_filters( 'scv2_add_to_cart_validation', true, $product_id, $quantity, $variation_id, $variation, $cart_item_data, $product_type );
		...
		if ( ! $passed_validation ) {
			...
			return new WP_Error( 'scv2_product_failed_validation', $message, array( 'status' => 404 ) );
		}

		/**
		 * Filters the quantity for specified products.
		 */
		$quantity = apply_filters( 'scv2_add_to_cart_quantity', $quantity, $product_id, $variation_id, $variation, $cart_item_data );

		// Load cart item data - may be added by other plugins.
		$cart_item_data = (array) apply_filters( 'scv2_add_cart_item_data', $cart_item_data, $product_id, $variation_id, $quantity, $product_type );
		...
		if ( $product->is_sold_individually() ) {
			...
			$found_in_cart = apply_filters( 'scv2_add_to_cart_sold_individually_found_in_cart', $cart_item_key && $cart_contents[ $cart_item_key ]['quantity'] > 0, $product_id, $variation_id, $cart_item_data, $cart_id );
			...
			if ( $found_in_cart ) {
				...
				return new WP_Error( 'scv2_product_sold_individually', $message, array( 'status' => 403 ) );
			}
		}
		...
		if ( ! $product->is_purchasable() ) {
			...
			return new WP_Error( 'scv2_cannot_be_purchased', $message, array( 'status' => 403 ) );
		}
		...
		if ( ! $product->is_in_stock() ) {
			...
			return new WP_Error( 'scv2_product_out_of_stock', $message, array( 'status' => 404 ) );
		}
		...
		if ( ! $product->has_enough_stock( $quantity ) ) {
			...
			return new WP_Error( 'scv2_not_enough_in_stock', $message, array( 'status' => 403 ) );
		}
		...
		if ( $product->managing_stock() ) {
			...
			if ( isset( $products_qty_in_cart[ $product->get_stock_managed_by_id() ] ) && ! $product->has_enough_stock( $products_qty_in_cart[ $product->get_stock_managed_by_id() ] + $quantity ) ) {
				...
				return new WP_Error( 'scv2_not_enough_stock_remaining', $message, array( 'status' => 403 ) );
			}
		}

		$response  = apply_filters( 'scv2_ok_to_add_response', '', $product, $product_id, $quantity );
		$ok_to_add = apply_filters( 'scv2_ok_to_add', true, $product, $product_id, $quantity );
		...
		if ( ! $ok_to_add ) {
			...
			SCV2_Logger::log( $error_msg, 'error' );

			return new WP_Error( 'scv2_not_ok_to_add_item', $error_msg, array( 'status' => 403 ) );
		}
	...
	public function get_response( $response, $rest_base = '' ) {
		...
		if ( $rest_base !== 'cart' && empty( $response ) ) {
			...
			SCV2_Logger::log( $response, 'error' );
		}
