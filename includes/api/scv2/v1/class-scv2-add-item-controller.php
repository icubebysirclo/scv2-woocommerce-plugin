 ...
 */
class SCV2_Add_Item_Controller extends SCV2_API_Controller {
	...
	public function register_routes() {
		// Add Item - scv2/v1/add-item (POST)
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base,
			array(
	...
	public function add_to_cart( $data = array() ) {
		...
		$add_to_cart_handler = apply_filters( 'scv2_add_to_cart_handler', $adding_to_cart->get_type(), $adding_to_cart );
		...
		} elseif ( has_filter( 'scv2_add_to_cart_handler_' . $add_to_cart_handler ) ) {
			$was_added_to_cart = apply_filters( 'scv2_add_to_cart_handler_' . $add_to_cart_handler, $adding_to_cart ); // Custom handler.
		} else {
	...
	public function add_item_to_cart( $product_to_add = array() ) {
		...
		if ( ! empty( $cart_item_key ) ) {
			 ...
			 */
			do_action( 'scv2_item_added_updated_in_cart', $cart_item_key, $item_added, $new_quantity );
		} else {
			// Add item to cart.
			...
			if ( $item_key ) {
				...
				do_action( 'scv2_item_added_to_cart', $item_key, $item_added );
			} else {
				 ...
				 */
				$message = apply_filters( 'scv2_product_cannot_add_to_cart_message', $message, $product_data );

				return new WP_Error( 'scv2_cannot_add_to_cart', $message, array( 'status' => 403 ) );
			}
		}
	...
	public function return_additional_item_data( $item_added, $item_key = '' ) {
		...
		$item_added['product_name']  = apply_filters( 'scv2_item_added_product_name', $_product->get_name(), $_product, $item_key );
		$item_added['product_title'] = apply_filters( 'scv2_item_added_product_title', $_product->get_title(), $_product, $item_key );
		...
		$item_added = apply_filters( 'scv2_item_added', $item_added, $item_key );
	...
	public function get_item_schema() {
		$schema = array(
			...
			'title'      => 'SCV2 - ' . __( 'Add Item', 'cart-rest-api-for-woocommerce' ),
			'type'       => 'object',
			'properties' => array(
				'product_id'     => array(
					'required'    => true,
		...
		$schema['properties'] = apply_filters( 'scv2_add_item_schema', $schema['properties'] );
