<?php
/**
 * SCV2 REST API Session controller.
 */
...
/**
 * SCV2 REST API Session v2 controller class.
 */
class SCV2_Session_V2_Controller extends SCV2_Cart_V2_Controller {
	...
	public function register_routes() {
		// Get Cart in Session - scv2/v2/session/ec2b1f30a304ed513d2975b7b9f222f6 (GET).
		register_rest_route(
			$this->namespace,
		...
		// Delete Cart in Session - scv2/v2/session/ec2b1f30a304ed513d2975b7b9f222f6 (DELETE).
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/(?P<cart_key>[\w]+)',
			array(
		...
		// Get Cart Items in Session - scv2/v2/session/ec2b1f30a304ed513d2975b7b9f222f6/items (GET).
		register_rest_route(
			$this->namespace,
	...
	public function get_cart_in_session( $request = array() ) {
		...
		try {
			...
			if ( empty( $cart_key ) ) {
				throw new SCV2_Data_Exception( 'scv2_cart_key_missing', __( 'Cart Key is required!', 'cart-rest-api-for-woocommerce' ), 404 );
			}
			...
			if ( empty( $cart ) ) {
				throw new SCV2_Data_Exception( 'scv2_cart_in_session_not_valid', __( 'Cart in session is not valid!', 'cart-rest-api-for-woocommerce' ), 404 );
			}

			return SCV2_Response::get_response( $this->return_cart_contents( $request, maybe_unserialize( $cart['cart'] ), '', true ), $this->namespace, $this->rest_base );
	...
	public function delete_cart( $request = array() ) {
		try {
			...
			if ( ! empty( $handler->get_cart( $cart_key ) ) ) {
				throw new SCV2_Data_Exception( 'scv2_cart_not_deleted', __( 'Cart could not be deleted!', 'cart-rest-api-for-woocommerce' ), 500 );
			}

			return SCV2_Response::get_response( __( 'Cart successfully deleted!', 'cart-rest-api-for-woocommerce' ), $this->namespace, $this->rest_base );
		} catch ( SCV2_Data_Exception $e ) {
			return SCV2_Response::get_error_response( $e->getErrorCode(), $e->getMessage(), $e->getCode(), $e->getAdditionalData() );
		}
	} // END delete_cart()
	...
	public function get_cart_items_in_session( $request = array() ) {
		...
		try {
			...
			return SCV2_Response::get_response( $this->get_items( maybe_unserialize( $cart['cart'] ), $show_thumb ), $this->namespace, $this->rest_base );
		} catch ( SCV2_Data_Exception $e ) {
			return SCV2_Response::get_error_response( $e->getErrorCode(), $e->getMessage(), $e->getCode(), $e->getAdditionalData() );
		}
