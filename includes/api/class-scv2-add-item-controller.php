 ...
 */
class SCV2_Add_Item_v2_Controller extends SCV2_Add_Item_Controller {
	...
	public function register_routes() {
		// Add Item - scv2/v2/cart/add-item (POST).
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base,
			array(
	...
	public function add_to_cart( $request = array() ) {
		try {
			...
			if ( 'variable' === $add_to_cart_handler || 'variation' === $add_to_cart_handler ) {
				...
				$was_added_to_cart = apply_filters( 'scv2_add_to_cart_handler_' . $add_to_cart_handler, $adding_to_cart, $request ); // Custom handler.
			} else {
				$was_added_to_cart = $this->add_to_cart_handler_simple( $product_id, $quantity, $item_data, $request );
			}

			if ( ! is_wp_error( $was_added_to_cart ) ) {
				scv2_add_to_cart_message( array( $was_added_to_cart['product_id'] => $was_added_to_cart['quantity'] ) );
	...
	public function add_item_to_cart( $product_to_add = array() ) {
		...
		try {
			...
			if ( ! empty( $item_key ) ) {
				 ...
				 */
				do_action( 'scv2_item_added_updated_in_cart', $item_key, $item_added, $new_quantity, $request );
			} else {
				/**
				 * Add item to cart without WC product validation.
				 */
				if ( apply_filters( 'scv2_skip_woocommerce_item_validation', false, $product_data, $product_id ) ) {
				...
				if ( $item_key ) {
					 ...
					 */
					do_action( 'scv2_item_added_to_cart', $item_key, $item_added, $request );
				} else {
					/**
					 * If WooCommerce can provide a reason for the error then let that error message return first.
					 */
					$controller->convert_notices_to_exceptions( 'scv2_add_to_cart_error' );
					...
					throw new SCV2_Data_Exception( 'scv2_cannot_add_to_cart', $message, 403 );
				}
			}
	...
	public function get_item_schema() {
		...
		$schema['properties'] = apply_filters( 'scv2_add_item_schema', $schema['properties'], $this->rest_base );
