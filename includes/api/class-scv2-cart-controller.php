...
/**
 * SCV2 REST API v2 controller class.
 */
class SCV2_Cart_V2_Controller extends SCV2_API_Controller {
	...
	public function get_cart_instance() {
		...
		if ( ! $cart || ! $cart instanceof \WC_Cart ) {
			throw new SCV2_Data_Exception( 'scv2_cart_error', __( 'Unable to retrieve cart.', 'cart-rest-api-for-woocommerce' ), 500 );
		}
	...
	public function get_cart_item( $item_id, $condition = 'add' ) {
		...
		return apply_filters( 'scv2_get_cart_item', $item, $condition );
	} // EMD get_cart_item()
	...
	public function return_cart_contents( $request = array(), $cart_contents = array(), $deprecated = '', $from_session = false ) {
		...
		if ( $this->get_cart_instance()->get_cart_contents_count() <= 0 && count( $this->get_cart_instance()->get_removed_cart_contents() ) <= 0 ) {
			 ...
			 */
			scv2_deprecated_filter( 'scv2_return_empty_cart', array(), '1.0.0', 'scv2_empty_cart', __( 'But only if you are using API v2.', 'cart-rest-api-for-woocommerce' ) );

			return apply_filters( 'scv2_empty_cart', array() );
		}
		...
		if ( wc_tax_enabled() && ! $this->get_cart_instance()->display_prices_including_tax() ) {
			...
			if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) {
				...
				$cart['taxes'] = array(
					...
					'total' => apply_filters( 'scv2_cart_totals_taxes_total', $this->prepare_money_response( $this->get_cart_instance()->get_taxes_total() ) ),
				);
			}
		}
		...
		return apply_filters( 'scv2_cart', $cart, $from_session );
		...
		try {
			...
			if ( ! is_numeric( $product_id ) ) {
				...
				if ( ! empty( $product_id_by_sku ) && $product_id_by_sku > 0 ) {
					...
					throw new SCV2_Data_Exception( 'scv2_unknown_product_id', $message, 500 );
				}
			}
			...
			if ( empty( $product_id ) ) {
				...
				throw new SCV2_Data_Exception( 'scv2_product_id_required', $message, 404 );
			}

			if ( ! is_numeric( $product_id ) ) {
				$message = __( 'Product ID must be numeric!', 'cart-rest-api-for-woocommerce' );

				throw new SCV2_Data_Exception( 'scv2_product_id_not_numeric', $message, 405 );
			}
	...
	protected function validate_quantity( $quantity ) {
		try {
			if ( ! is_numeric( $quantity ) ) {
				throw new SCV2_Data_Exception( 'scv2_quantity_not_numeric', __( 'Quantity must be integer or a float value!', 'cart-rest-api-for-woocommerce' ), 405 );
			}

			/**
			 * This filter was added to support certain edge cases.
			 */
			$minimum_quantity = apply_filters( 'scv2_quantity_minimum_requirement', 1 );

			if ( 0 == $quantity || $quantity < $minimum_quantity ) {
				throw new SCV2_Data_Exception( 'scv2_quantity_invalid_amount', sprintf( __( 'Quantity must be set to a minimum of %s.', 'cart-rest-api-for-woocommerce' ), $minimum_quantity ), 405 );
			}
		...
		try {
			...
			foreach ( $variable_product_attributes as $attribute ) {
				...
				if ( isset( $variation[ wc_variation_attribute_name( $attribute['name'] ) ] ) ) {
					 ...
					 */
					$message = apply_filters( 'scv2_invalid_variation_data_message', $message, $attribute_label, $attribute->get_slugs() );

					throw new SCV2_Data_Exception( 'scv2_invalid_variation_data', $message, 400 );
				}
			...
			if ( ! empty( $missing_attributes ) ) {
				 ...
				 */
				$message = apply_filters( 'scv2_missing_variation_data_message', $message, count( $missing_attributes ), wc_format_list_of_items( $missing_attributes ) );

				throw new SCV2_Data_Exception( 'scv2_missing_variation_data', $message, 400 );
			}
	...
	protected function get_variation_id_from_variation_data( $variation, $product ) {
		try {
			...
			if ( empty( $variation_id ) ) {
				...
				throw new SCV2_Data_Exception( 'scv2_no_variation_found', $message, 404 );
			}
	...
	protected function get_variable_product_attributes( $product ) {
		try {
			...
			if ( ! $product || 'trash' === $product->get_status() ) {
				...
				throw new SCV2_Data_Exception( 'scv2_cart_invalid_parent_product', $message, 403 );
			}
	...
	protected function validate_product( $product_id = null, $quantity = 1, $deprecated = null, $variation = array(), $item_data = array(), $product_type = '', $request = array() ) {
		try {
			...
			$passed_validation = apply_filters( 'scv2_add_to_cart_validation', true, $product_id, $quantity, $variation_id, $variation, $item_data, $product_type, $request );
			...
			if ( ! $passed_validation ) {
				 ...
				 */
				$message = apply_filters( 'scv2_product_failed_validation_message', $message, $product );

				throw new SCV2_Data_Exception( 'scv2_product_failed_validation', $message, 500 );
			}

			// Add cart item data - may be added by other plugins.
			$item_data = (array) apply_filters( 'scv2_add_cart_item_data', $item_data, $product_id, $variation_id, $quantity, $product_type, $request );
			 ...
			 */
			$quantity = apply_filters( 'scv2_add_to_cart_quantity', $quantity, $product_id, $variation_id, $variation, $item_data );
	...
	protected function has_enough_stock( $current_data = array(), $quantity = 1 ) {
		try {
			...
			if ( ! $current_product->has_enough_stock( $quantity ) ) {
				...
				throw new SCV2_Data_Exception( 'scv2_not_enough_in_stock', $message, array( 'status' => 403 ) );
			}
	...
	public function get_cart_key( $request ) {
		if ( ! class_exists( 'SCV2_Session_Handler' ) || ! WC()->session instanceof SCV2_Session_Handler ) {
			return;
		}
	...
	protected function format_variation_data( $variation_data, $product ) {
		...
		foreach ( $variation_data as $key => $value ) {
			...
			if ( taxonomy_exists( $taxonomy ) ) {
				...
				$value = apply_filters( 'scv2_variation_option_name', $value, $product );
	...
	public function coupon_html( $coupon, $formatted = true ) {
		...
		$discount_amount_html = apply_filters( 'scv2_coupon_discount_amount_html', $discount_amount_html, $coupon );
	...
	public function fee_html( $cart, $fee ) {
		...
		return apply_filters( 'scv2_cart_totals_fee_html', $cart_totals_fee_html, $fee );
	} // END fee_html()
	...
	public function validate_product_for_cart( $product ) {
		try {
			...
			if ( ! $product || 'trash' === $product->get_status() ) {
				 ...
				 */
				$message = apply_filters( 'scv2_product_cannot_be_added_message', $message, $product );

				throw new SCV2_Data_Exception( 'scv2_invalid_product', $message, 400 );
			}
	...
	public function validate_item_quantity( $product, $quantity, $product_id, $variation_id, $item_data, $cart_id ) {
		try {
			...
			if ( $product->is_sold_individually() ) {
				 ...
				 */
				$quantity = apply_filters( 'scv2_add_to_cart_sold_individually_quantity', 1 );

				$cart_contents = $this->get_cart();

				$found_in_cart = apply_filters( 'scv2_add_to_cart_sold_individually_found_in_cart', $item_key && $cart_contents[ $item_key ]['quantity'] > 0, $product_id, $variation_id, $item_data, $cart_id );
				...
				if ( $found_in_cart ) {
					 ...
					 */
					$message = apply_filters( 'scv2_product_can_not_add_another_message', $message, $product );

					throw new SCV2_Data_Exception( 'scv2_product_sold_individually', $message, 403 );
				}
			}
	...
	public function validate_add_to_cart( $product, $quantity ) {
		try {
			...
			if ( ! $product->is_in_stock() ) {
				 ...
				 */
				$message = apply_filters( 'scv2_product_is_out_of_stock_message', $message, $product );

				throw new SCV2_Data_Exception( 'scv2_product_out_of_stock', $message, 404 );
			}
			...
			if ( ! $product->has_enough_stock( $quantity ) ) {
				...
				throw new SCV2_Data_Exception( 'scv2_not_enough_in_stock', $message, 403 );
			}
			...
			if ( $product->managing_stock() && ! $product->backorders_allowed() ) {
				...
				if ( isset( $qty_in_cart[ $product->get_stock_managed_by_id() ] ) && ! $product->has_enough_stock( $qty_in_cart[ $product->get_stock_managed_by_id() ] + $quantity ) ) {
					...
					throw new SCV2_Data_Exception( 'scv2_not_enough_stock_remaining', $message, 403 );
				}
			}

			scv2_deprecated_hook( 'scv2_ok_to_add_response', '1.0.0', null, 'This filter is no longer used in the API.' );
			scv2_deprecated_hook( 'scv2_ok_to_add', '1.0.0', null, 'This filter is no longer used in the API.' );
		} catch ( SCV2_Data_Exception $e ) {
			return SCV2_Response::get_error_response( $e->getErrorCode(), $e->getMessage(), $e->getCode(), $e->getAdditionalData() );
		}
	...
	public function filter_request_data( $request ) {
		return apply_filters( 'scv2_filter_request_data', $request );
	...
	public function get_item( $_product, $cart_item = array(), $item_key = '', $show_thumb = true, $removed_item = false ) {
		$item = array(
			'item_key'       => $item_key,
			'id'             => $_product->get_id(),
			'name'           => apply_filters( 'scv2_cart_item_name', $_product->get_name(), $_product, $cart_item, $item_key ),
			'title'          => apply_filters( 'scv2_cart_item_title', $_product->get_title(), $_product, $cart_item, $item_key ),
			'price'          => apply_filters( 'scv2_cart_item_price', wc_format_decimal( $_product->get_price(), wc_get_price_decimals() ), $cart_item, $item_key ),
			'quantity'       => array(
				'value'        => apply_filters( 'scv2_cart_item_quantity', $cart_item['quantity'], $item_key, $cart_item ),
			...
			'totals'         => array(
				'subtotal'     => apply_filters( 'scv2_cart_item_subtotal', $cart_item['line_subtotal'], $cart_item, $item_key ),
		...
		if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
			$item['backorders'] = wp_kses_post( apply_filters( 'scv2_cart_item_backorder_notification', esc_html__( 'Available on backorder', 'cart-rest-api-for-woocommerce' ), $_product->get_id() ) );
		}
		...
		if ( ! empty( $cart_item ) ) {
			$item['cart_item_data'] = apply_filters( 'scv2_cart_item_data', $cart_item, $item_key, $cart_item );
		}
		...
		if ( $show_thumb ) {
			...
			$thumbnail_id = apply_filters( 'scv2_item_thumbnail', $thumbnail_id, $cart_item, $item_key, $removed_item );

			$thumbnail_src = wp_get_attachment_image_src( $thumbnail_id, apply_filters( 'scv2_item_thumbnail_size', 'woocommerce_thumbnail', $removed_item ) );
			 ...
			 */
			$thumbnail_src = apply_filters( 'scv2_item_thumbnail_src', $thumbnail_src, $cart_item, $item_key, $removed_item );
	...
	public function get_items( $cart_contents = array(), $show_thumb = true ) {
		...
		foreach ( $cart_contents as $item_key => $cart_item ) {
			...
			$_product = apply_filters( 'scv2_item_product', $cart_item['data'], $cart_item, $item_key );
			...
			if ( ! $_product->is_purchasable() ) {
				 ...
				 */
				$message = apply_filters( 'scv2_cart_item_removed_message', $message, $_product );
				...
				$items = apply_filters( 'scv2_cart_items', $items, $item_key, $cart_item, $_product );
			}
		}
	...
	public function get_cross_sells() {
		...
		$orderby         = apply_filters( 'scv2_cross_sells_orderby', 'rand' );
		$order           = apply_filters( 'scv2_cross_sells_order', 'desc' );
		$get_cross_sells = wc_products_array_orderby( $get_cross_sells, $orderby, $order );
		$limit           = apply_filters( 'scv2_cross_sells_total', 3 );
		...
		foreach ( $get_cross_sells as $cross_sell ) {
			...
			$thumbnail_id  = apply_filters( 'scv2_cross_sell_item_thumbnail', $cross_sell->get_image_id() );
			$thumbnail_src = wp_get_attachment_image_src( $thumbnail_id, apply_filters( 'scv2_cross_sell_item_thumbnail_size', 'woocommerce_thumbnail' ) );
			$thumbnail_src = apply_filters( 'scv2_item_thumbnail_src', $thumbnail_src[0] );
		...
		$cross_sells = apply_filters( 'scv2_cross_sells', $cross_sells );
	...
	public function get_shipping_details() {
		...
		foreach ( $packages as $i => $package ) {
			...
			if ( count( (array) $packages ) > 1 ) {
				...
				$product_names = apply_filters( 'scv2_shipping_package_details_array', $product_names, $package );
			}
			...
			$details['packages'][ $package_key ] = array(
				...
				'package_name'          => apply_filters( 'scv2_shipping_package_name', ( ( $i + 1 ) > 1 ) ? sprintf( _x( 'Shipping #%d', 'shipping packages', 'cart-rest-api-for-woocommerce' ), ( $i + 1 ) ) : _x( 'Shipping', 'shipping packages', 'cart-rest-api-for-woocommerce' ), $i, $package ),
				'rates'                 => $package['rates'],
	...
	protected function print_notices() {
		$all_notices  = WC()->session->get( 'wc_notices', array() );
		$notice_types = apply_filters( 'scv2_notice_types', array( 'error', 'success', 'notice', 'info' ) );
		$notices      = array();
	...
	public function add_cart_item( $product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array() ) {
		try {
			...
			$this->get_cart_instance()->cart_contents = apply_filters( 'scv2_cart_contents_changed', $this->get_cart_instance()->cart_contents );

			do_action( 'scv2_add_to_cart', $item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data );
	...
	protected function get_customer( $fields = 'billing' ) {
		...
		foreach ( $checkout_fields as $key => $value ) {
			...
			$results[ $key ] = method_exists( $customer, $field_name ) ? $customer->$field_name() : apply_filters( 'scv2_get_customer_' . $key, '' );
		}
	...
	public static function convert_notices_to_exceptions( $error_code = 'unknown_server_error' ) {
		...
		foreach ( $error_notices as $error_notice ) {
			throw new SCV2_Data_Exception( $error_code, wp_strip_all_tags( $error_notice['notice'] ), 400 );
		}
	...
	protected function throw_product_not_purchasable( $product ) {
		 ...
		 */
		$message = apply_filters( 'scv2_product_cannot_be_purchased_message', $message, $product );

		throw new SCV2_Data_Exception( 'scv2_cannot_be_purchased', $message, 403 );
	...
	protected function throw_missing_item_key( $item_key, $status ) {
		...
		if ( '0' === $item_key ) {
			 ...
			 */
			$message = apply_filters( 'scv2_cart_item_key_required_message', $message, $status );

			throw new SCV2_Data_Exception( 'scv2_cart_item_key_required', $message, 404 );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'title'      => 'SCV2 - ' . __( 'Cart', 'cart-rest-api-for-woocommerce' ),
			'type'       => 'object',
			'properties' => array(
				'items' => array(
		...
		$schema['properties'] = apply_filters( 'scv2_cart_schema', $schema['properties'] );
