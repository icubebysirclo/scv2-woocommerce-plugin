...
/**
 * SCV2 REST API session class.
 */
class SCV2_API_Session {
	...
	public function __construct() {
		...
		add_action( 'scv2_cleanup_carts', array( $this, 'cleanup_carts' ) );
	...
	public static function clear_carts() {
		...
		$wpdb->query( "TRUNCATE {$wpdb->prefix}scv2_carts" );
	...
	public static function cleanup_carts() {
		if ( class_exists( 'SCV2_Session_Handler' ) ) {
			$handler = new SCV2_Session_Handler();
		...
		if ( self::maybe_load_cart() ) {
			...
			if ( is_user_logged_in() ) {
				...
				if ( $user_id === $cart_key ) {
					...
					SCV2_Logger::log( sprintf( __( 'Cart key "%s" is already loaded as the user is logged in.', 'cart-rest-api-for-woocommerce' ), $cart_key ), 'errro' );
				} else {
					/* translators: %s: cart key */
					SCV2_Logger::log( sprintf( __( 'Customer is already logged in. Cart key "%s" cannot be loaded into session.', 'cart-rest-api-for-woocommerce' ), $cart_key ), 'error' );
				}
				return;
			} else {
				...
				if ( ! empty( $user ) ) {
					SCV2_Logger::log( __( 'Cart key is recognised as a registered user on site. Cannot be loaded into session.', 'cart-rest-api-for-woocommerce' ), 'error' );
					return;
				}
			}
			...
			if ( empty( $stored_cart ) ) {
				/* translators: %s: cart key */
				SCV2_Logger::log( sprintf( __( 'Unable to find cart for: %s', 'cart-rest-api-for-woocommerce' ), $cart_key ), 'info' );
			...
			if ( $override_cart ) {
				...
				if ( ! WC()->cart->is_empty() ) {
					...
					do_action( 'scv2_load_cart_override', $new_cart, $stored_cart );
				}
			} else {
				$new_cart_content                       = array_merge( $new_cart['cart'], $cart_in_session );
				$new_cart['cart']                       = apply_filters( 'scv2_merge_cart_content', $new_cart_content, $new_cart['cart'], $cart_in_session );
				...
				do_action( 'scv2_load_cart', $new_cart, $stored_cart, $cart_in_session );
			}
			...
			if ( ! empty( $new_cart ) && $notify_customer ) {
				...
				wc_add_notice( apply_filters( 'scv2_cart_loaded_successful_message', sprintf( __( 'Your 🛒 cart has been transferred over. You may %1$scontinue shopping%3$s or %2$scheckout%3$s.', 'cart-rest-api-for-woocommerce' ), '<a href="' . wc_get_page_permalink( 'shop' ) . '">', '<a href="' . wc_get_checkout_url() . '">', '</a>' ) ), 'notice' );
			}
		}
	...
	public static function maybe_load_cart() {
		...
		if ( apply_filters( 'scv2_disable_load_cart', false ) ) {
			return false;
		}
	...
	protected static function get_action_query() {
		 ...
		 */
		$load_cart = apply_filters( 'scv2_load_cart_query_name', 'scv2-load-cart' );
