...
if ( ! class_exists( 'SCV2_Cart_Validation' ) ) {

	class SCV2_Cart_Validation {

		/**
		 * Constructor.
		 */
		public function __construct() {
			add_filter( 'scv2_before_get_cart', array( $this, 'check_cart_item_stock' ), 10, 2 );
			add_filter( 'scv2_before_get_cart', array( $this, 'check_cart_coupons' ), 15, 2 );
		}
		...
		public function check_cart_item_stock( $cart_contents, $cart ) {
			...
			foreach ( $cart_contents as $item_key => $values ) {
				 ...
				 */
				if ( apply_filters( 'scv2_cart_item_required_stock_is_not_enough', $product->get_stock_quantity() < ( $held_stock + $required_stock ), $product, $values ) ) {
					if ( ! $item_has_error ) {
...
return new SCV2_Cart_Validation();
