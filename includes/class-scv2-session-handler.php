...
// Checks that SCV2 session abstract exists first.
if ( ! class_exists( 'SCV2_Session' ) ) {
	return;
}
 ...
 */
class SCV2_Session_Handler extends SCV2_Session {
	...
	public function __construct() {
		$this->_cookie = apply_filters( 'scv2_cookie', 'wp_scv2_session_' . COOKIEHASH );
		$this->_table  = $GLOBALS['wpdb']->prefix . 'scv2_carts';
	}
	...
	public function init() {
		 ...
		 * @version 2.3.0
		 */
		if ( SCV2_Authentication::is_rest_api_request() && is_numeric( $current_user_id ) && $current_user_id < 1 ) {
			add_filter( 'nonce_user_logged_out', array( $this, 'nonce_user_logged_out' ) );
		}

		/**
		 * Identifies the source of the cart if it was created
		 * via SCV2 REST API or via the frontend a.k.a "WooCommerce".
		 *
		 * @since 1.0.0
		 */
		if ( SCV2_Authentication::is_rest_api_request() ) {
			$this->_cart_source = 'scv2';
		} else {
	...
	public function init_session_cookie( $current_user_id = 0 ) {
		...
		$override_cookie_check = apply_filters( 'scv2_override_cookie_check', false );
	...
	public function is_cookie_supported() {
		return apply_filters( 'scv2_cookie_supported', true );
	...
	public function set_customer_cart_cookie( $set = true ) {
		...
		if ( $set ) {
			...
			if ( ! isset( $_COOKIE[ $this->_cookie ] ) || $_COOKIE[ $this->_cookie ] !== $cookie_value ) {
				$this->scv2_setcookie( $this->_cookie, $cookie_value, $this->_cart_expiration, $this->use_secure_cookie(), $this->use_httponly() );
			}
		} else {
			// If cookies exists, destroy it.
			if ( isset( $_COOKIE[ $this->_cookie ] ) ) {
				$this->scv2_setcookie( $this->_cookie, '', time() - YEAR_IN_SECONDS, $this->use_secure_cookie(), $this->use_httponly() );
	...
	protected function use_secure_cookie() {
		return apply_filters( 'scv2_cart_use_secure_cookie', wc_site_is_https() && is_ssl() );
	} // END use_secure_cookie()

	/**
	 * Set a cookie - wrapper for setcookie using WP constants.
	 */
	public function scv2_setcookie( $name, $value, $expire = 0, $secure = false, $httponly = false ) {
		if ( ! headers_sent() ) {
			// samesite - Set to None by default and only available to those using PHP 7.3 or above. @since 2.9.1.
			if ( version_compare( PHP_VERSION, '7.3.0', '>=' ) ) {
				setcookie( $name, $value, apply_filters( 'scv2_set_cookie_options', array( 'expires' => $expire, 'secure' => $secure, 'path' => COOKIEPATH ? COOKIEPATH : '/', 'domain' => COOKIE_DOMAIN, 'httponly' => apply_filters( 'scv2_cookie_httponly', $httponly, $name, $value, $expire, $secure ), 'samesite' => apply_filters( 'scv2_cookie_samesite', 'Lax' ) ), $name, $value ) ); // phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
			} else {
				setcookie( $name, $value, $expire, COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN, $secure, apply_filters( 'scv2_cookie_httponly', $httponly, $name, $value, $expire, $secure ) );
			}
	...
	public function set_cart_expiration() {
		$this->_cart_expiring   = time() + intval( apply_filters( 'scv2_cart_expiring', DAY_IN_SECONDS * 6 ) ); // 6 Days.
		$this->_cart_expiration = time() + intval( apply_filters( 'scv2_cart_expiration', DAY_IN_SECONDS * 7 ) ); // 7 Days.
	...
	public function generate_customer_id() {
		...
		if ( empty( $customer_id ) ) {
			...
			$customer_id = apply_filters( 'scv2_customer_id', md5( $hasher->get_random_bytes( 32 ) ), $hasher );
		}
	...
	public function get_cache_prefix() {
		return WC_Cache_Helper::get_cache_prefix( SCV2_CART_CACHE_GROUP );
	...
	public function save_cart( $old_cart_key = 0 ) {
		if ( $this->has_session() ) {
			...
			/**
			 * Deprecated filter: `scv2_empty_cart_expiration` as it is no longer needed.
			 */
			if ( has_filter( 'scv2_empty_cart_expiration' ) ) {
				/* translators: %s: filter name */
				$message = sprintf( __( 'This filter "%s" is no longer required and has been deprecated.', 'cart-rest-api-for-woocommerce' ), 'scv2_empty_cart_expiration' );
				scv2_deprecated_hook( 'scv2_empty_cart_expiration', '2.7.2', null, $message );
			}
			 ...
			 */
			$cart_source = apply_filters( 'scv2_cart_source', $this->_cart_source );
			...
			$wpdb->query(
				$wpdb->prepare(
					"INSERT INTO {$wpdb->prefix}scv2_carts (`cart_key`, `cart_value`, `cart_created`, `cart_expiry`, `cart_source`, `cart_hash`) VALUES (%s, %s, %d, %d, %s, %s)
			...
			wp_cache_set( $this->get_cache_prefix() . $this->_customer_id, $this->_data, SCV2_CART_CACHE_GROUP, $this->_cart_expiration - time() );
	...
	public function cleanup_sessions() {
		...
		if ( class_exists( 'WC_Cache_Helper' ) ) {
			WC_Cache_Helper::invalidate_cache_group( SCV2_CART_CACHE_GROUP );
		}
	...
	public function get_cart( $cart_key, $default = false ) {
		...
		$value = wp_cache_get( $this->get_cache_prefix() . $cart_key, SCV2_CART_CACHE_GROUP );
		...
		if ( false === $value ) {
			...
			if ( 0 < $cache_duration ) {
				wp_cache_add( $this->get_cache_prefix() . $cart_key, $value, SCV2_CART_CACHE_GROUP, $cache_duration );
			}
		}
	...
	public function create_new_cart( $cart_key = '', $cart_value = array(), $cart_expiration = '', $cart_source = '' ) {
		...
		if ( empty( $cart_expiration ) ) {
			$cart_expiration = time() + intval( apply_filters( 'scv2_cart_expiring', DAY_IN_SECONDS * 7 ) );
		}
	...
	public function delete_cart( $cart_key ) {
		...
		// Delete cache.
		wp_cache_delete( $this->get_cache_prefix() . $cart_key, SCV2_CART_CACHE_GROUP );
	...
	protected function is_cart_data_valid( $data, $cart_key ) {
		...
		$data = apply_filters( 'scv2_is_cart_data_valid', $data );
