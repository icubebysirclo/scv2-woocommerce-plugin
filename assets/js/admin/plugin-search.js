...
( function ( $ ) {
	SCV2PS = {
		...
		$addOns: $( 'body.scv2-plugin-install #plugin-filter' ),
		...
		getSuggestion: function () {
			return document.querySelector( '.plugin-card-scv2-plugin-search' );
		},
		 ...
		 */
		getCard: function () {
			return document.querySelectorAll( 'body.scv2-plugin-install .plugin-card:not(.plugin-card-scv2-plugin-search)' );
		},
		...
		updateCardTitle: function () {
			var hint = SCV2PS.getSuggestion();
			var card = SCV2PS.getCard();
		...
		replaceCardBottom: function () {
			...
			if ( 'object' === typeof hint && null !== hint ) {
				hint.querySelector( '.plugin-card-bottom' ).outerHTML =
					'<div class="scv2-plugin-search__bottom">' +
					'<p class="scv2-plugin-search__text">' +
					SCV2PluginSearch.legend +
					' <a class="scv2-plugin-search__support_link" href="' +
					SCV2PluginSearch.supportLink +
					'" target="_blank" rel="noopener noreferrer" data-track="support_link" >' +
					SCV2PluginSearch.supportText +
					'</a>' +
					'</p>' +
					'</div>';
			}
		...
		hideCoreCard: function ( ) {
			var core = document.querySelector( 'body.scv2-plugin-install .plugin-card.plugin-card-cart-rest-api-for-woocommerce' );
		...
		reset: function() {
			...
			var dashboard = document.querySelector( '.scv2-plugin-install-dashboard' )

			if ( $(body).hasClass( 'scv2-plugin-install' ) ) {
				$(body).removeClass( 'scv2-plugin-install' );
			}
		...
		/**
		 * Check if plugin card list nodes changed. If there's a SCV2 PSH card, replace the title and the bottom row.
		...
		replaceOnNewResults: function ( mutationsList ) {
			mutationsList.forEach( function ( mutation ) {
				if (
					...
					1 === document.querySelectorAll( '.plugin-card-scv2-plugin-search' ).length
				) {
					SCV2PS.reset();
					SCV2PS.unlinkCardTitle();
					SCV2PS.updateCardTitle();
					SCV2PS.moveActionLinks();
					SCV2PS.replaceCardBottom();
				}
			} );
		},
		 ...
		 */
		init: function () {
			if ( SCV2PS.$pluginFilter.length < 1 ) {
				return;
			}
			...
			// Update title to show that the suggestion is from SCV2.
			SCV2PS.updateCardTitle();
			...
			var resultsObserver = new MutationObserver( SCV2PS.replaceOnNewResults );
			resultsObserver.observe( document.getElementById( 'plugin-filter' ), { childList: true } );
		},
	};

	SCV2PS.init();
} )( jQuery, SCV2PluginSearch );
